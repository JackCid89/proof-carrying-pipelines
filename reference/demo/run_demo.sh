...
KEY_REF=$(python3 -c "import hashlib,pathlib;print('ed25519:sha256:'+hashlib.sha256(pathlib.Path('../demo-key.pub').read_bytes()).hexdigest()[:32])")
cat > ../approved.json <<EOF
{ "identities": ["demo@pcp.dev"],
  "keys": { "$KEY_REF": "demo@pcp.dev" } }
EOF
...
python3 $PCP verify --repo . --attestation ../attestation.json --pub ../demo-key.pub --approved ../approved.json
echo; echo "══ IMPERSONATION: same key, someone else's identity (expect RUN FULL — P5/V2) ══"
python3 $PCP attest --repo . --gates gates.yaml --key ../demo-key.key --identity mallory@pcp.dev --out ../impostor.json
if python3 $PCP verify --repo . --attestation ../impostor.json --pub ../demo-key.pub --approved ../approved.json; then
  echo "UNEXPECTED: impersonated identity passed"; exit 1
else
  echo "✓ identity-to-key binding enforced (P5)"
fi
